...
export function AppProvider({ children }) {
  const { user } = useAuth();
  const storageKey = user ? `gunluk-takip-v1-${user.uid}` : 'gunluk-takip-v1';
  const [state, setState] = useLocalStorage(storageKey, DEFAULT_STATE);
  ...
  // ── EVENTS ─────────────────────────────────────────────
  const addEvent = (data) => update('events', events => [
    ...events, { id: genId(), ...data }
  ]);
