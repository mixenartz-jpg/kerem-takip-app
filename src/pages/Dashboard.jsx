...
import {
  CheckSquare, Clock, Activity, FolderKanban, Flame, TrendingUp,
  BookOpen, ClipboardList, Target, AlertCircle, ArrowRight, Zap,
  Brain, Sparkles, ListTodo, Plus, GraduationCap, BarChart2, Timer,
} from 'lucide-react';
...
function SectionCard({ title, icon: Icon, iconColor = 'text-violet-400', action, actionPath, children }) {
  ...
  return (
    <motion.div variants={cardVariants} className="relative overflow-hidden rounded-2xl group">
...
/* ── AKADEMI DASHBOARD ── */
function AkademiDashboard() {
  const { lessons, exams, habits, pomodoro, yks } = useApp();
  const navigate = useNavigate();
  ...
  const yksExamDate = yks?.examDate;
  const daysToYks = yksExamDate
    ? Math.max(0, differenceInDays(parseISO(yksExamDate), new Date()))
    : null;

  // Net summary from last trial
  const lastTrial = useMemo(() => {
  ...
  return (
    <motion.div className="p-4 md:p-6 space-y-5 min-h-full" initial="hidden" animate="show" variants={container}>
      ...
      >
        ...
        <div className="flex items-center justify-between">
          <div className="flex items-start gap-3">
            ...
            <p className="text-sm text-emerald-200/80 italic leading-relaxed">"{QUOTE}"</p>
          </div>
          {daysToYks !== null && (
            <div className="shrink-0 text-right ml-4">
              <div className="text-2xl font-bold text-emerald-400 font-mono">{daysToYks}</div>
              <div className="text-[10px] text-zinc-500 uppercase tracking-wide">gün kaldı</div>
            </div>
          )}
        </div>
      </motion.div>
      ...
      <motion.div variants={container} className="grid grid-cols-2 xl:grid-cols-4 gap-3">
        <StatCard icon={BookOpen} label="Toplam çalışma" value={`${totalStudyHours}h`}
          sub={`${lessons.length} ders`} color="violet" delay={0.05}
          onClick={() => navigate('/yks')} />
        <StatCard icon={Timer} label="Bugün Pomodoro" value={pomodoroToday}
          sub="seans tamamlandı" color="blue" delay={0.1}
          onClick={() => navigate('/pomodoro')} />
        <StatCard icon={Activity} label="Alışkanlıklar" value={`${habitsToday.done}/${habitsToday.total}`}
          sub="bugün" color="green" delay={0.15}
          onClick={() => navigate('/habits')} />
        <StatCard icon={ClipboardList} label="Yaklaşan Sınav" value={upcomingExams.length}
          sub="sınav var" color="red" delay={0.2}
          onClick={() => navigate('/yks')} />
      </motion.div>
      ...
      <motion.div variants={cardVariants}>
        <div className="grid grid-cols-3 gap-3">
          {[
            { to: '/yks', icon: Brain, label: 'YKS Merkezi', color: 'text-emerald-300', bg: 'from-emerald-600/20 to-emerald-500/10', border: 'border-emerald-500/20' },
            { to: '/planner', icon: Sparkles, label: 'AI Planlayıcı', color: 'text-violet-300', bg: 'from-violet-600/20 to-violet-500/10', border: 'border-violet-500/20' },
            { to: '/pomodoro', icon: Timer, label: 'Pomodoro', color: 'text-blue-300', bg: 'from-blue-600/20 to-blue-500/10', border: 'border-blue-500/20' },
      ...
      <motion.div variants={container} className="grid grid-cols-1 lg:grid-cols-2 gap-4">
        ...
        <SectionCard title="Yaklaşan Sınavlar" icon={ClipboardList} iconColor="text-red-400"
          action="YKS Merkezi" actionPath="/yks">
                ...
                {upcomingExams.map((exam, i) => {
                  ...
                  return (
                    <motion.li key={exam.id} initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: i * 0.06 }}
                      className={`flex items-center gap-3 py-1.5 px-3 rounded-xl transition-colors ${urgent ? 'bg-red-500/8 border border-red-500/15' : ''}`}>
        ...
        {/* Son Deneme Sonucu */}
        <SectionCard title="Son Deneme Sonucu" icon={BarChart2} iconColor="text-emerald-400"
          action="Tüm Denemeler" actionPath="/yks">
          {!lastTrial
            ? <EmptyState text="Henüz deneme eklenmedi" />
            : <div className="space-y-3">
                <div className="flex items-center justify-between mb-2">
                  <span className="text-xs text-zinc-500">{lastTrial.name || 'Son deneme'}</span>
                  <span className="text-xs text-zinc-500 font-mono">{lastTrial.date}</span>
                </div>
                {Object.entries(lastTrial.nets || {}).slice(0, 4).map(([key, val]) => (
                  <div key={key} className="flex items-center gap-3">
                    <span className="text-xs text-zinc-500 w-24 shrink-0">{key.replace('_', ' ').toUpperCase()}</span>
                    <div className="flex-1 h-1.5 bg-zinc-800 rounded-full overflow-hidden">
                      <motion.div
                        className="h-full bg-gradient-to-r from-emerald-600 to-emerald-400 rounded-full"
                        initial={{ width: 0 }}
                        animate={{ width: `${Math.min(100, (val / 40) * 100)}%` }}
                        transition={{ duration: 0.8, ease: [0.23, 1, 0.32, 1] }}
                      />
                    </div>
                    <span className="text-xs font-mono text-emerald-400 w-8 text-right">{val}</span>
                  </div>
                ))}
              </div>
          }
        ...
        {/* Konu Tamamlama */}
        <SectionCard title="Konu İlerlemesi" icon={TrendingUp} iconColor="text-violet-400"
          action="Konular" actionPath="/yks">
          {!topicProgress
            ? <EmptyState text="Konu takibi başlatılmadı" />
            : <div className="space-y-3">
                <div className="flex items-center justify-between">
                  <span className="text-2xl font-bold text-violet-400 font-mono">{topicProgress.pct}%</span>
                  <span className="text-xs text-zinc-500">{topicProgress.done}/{topicProgress.total} konu</span>
                </div>
                <div className="h-2 bg-zinc-800 rounded-full overflow-hidden">
                  <motion.div
                    className="h-full bg-gradient-to-r from-violet-600 to-purple-400 rounded-full"
                    initial={{ width: 0 }}
                    animate={{ width: `${topicProgress.pct}%` }}
                    transition={{ duration: 1, ease: [0.23, 1, 0.32, 1] }}
                  />
                </div>
                <p className="text-xs text-zinc-600 text-center">
                  {topicProgress.pct >= 80 ? '🎉 Harika ilerleme!' : topicProgress.pct >= 50 ? '💪 Yarısını geçtin!' : '📚 Devam et!'}
                </p>
              </div>
          }
        </SectionCard>
                  ...
                  return (
                    <motion.li key={h.id} initial={{ opacity: 0, x: -8 }} animate={{ opacity: 1, x: 0 }}
                      ...
                      <div className={`w-5 h-5 rounded-full border-2 flex items-center justify-center transition-all ${
                        ...
                        {done && <motion.span initial={{ scale: 0 }} animate={{ scale: 1 }} className="text-white text-[10px] font-bold">✓</motion.span>}
                      </div>
                    </motion.li>
                  );
                })}
...
function GunlukDashboard() {
  const { tasks, habits, projects, pomodoro, goals, dailyTodos, addDailyTodo, toggleDailyTodo } = useApp();
    ...
    <motion.div className="p-4 md:p-6 space-y-5 min-h-full" initial="hidden" animate="show" variants={container}>
      ...
      <motion.div variants={container} className="grid grid-cols-2 xl:grid-cols-4 gap-3">
        ...
        <StatCard icon={Activity} label="Alışkanlıklar" value={`${habitsToday.done}/${habitsToday.total}`}
          ...
          onClick={() => navigate('/habits')} />
        <StatCard icon={Timer} label="Bugün Pomodoro" value={pomodoroToday}
          sub="seans" color="violet" delay={0.15}
          onClick={() => navigate('/pomodoro')} />
      ...
      <motion.div variants={cardVariants}>
        <div className="grid grid-cols-3 gap-3">
          {[
            { to: '/tasks', icon: ListTodo, label: 'Görevler', color: 'text-violet-300', bg: 'from-violet-600/20 to-violet-500/10', border: 'border-violet-500/20' },
            { to: '/ai', icon: Sparkles, label: 'AI Merkezi', color: 'text-pink-300', bg: 'from-pink-600/20 to-pink-500/10', border: 'border-pink-500/20' },
            { to: '/projects', icon: FolderKanban, label: 'Projeler', color: 'text-blue-300', bg: 'from-blue-600/20 to-blue-500/10', border: 'border-blue-500/20' },
          ].map(({ to, icon: Icon, label, color, bg, border }) => (
            <motion.button
              key={to}
              onClick={() => navigate(to)}
      ...
      <motion.div variants={container} className="grid grid-cols-1 lg:grid-cols-2 gap-4">
        ...
        <SectionCard title="Bugünkü Plan" icon={ListTodo} iconColor="text-violet-400"
          action="Görevler" actionPath="/tasks">
          ...
          {todayDailyTodos.length > 0 && (
            <ul className="space-y-1.5 mb-3">
              {todayDailyTodos.slice(0, 5).map((t, i) => (
                <motion.li key={t.id} initial={{ opacity: 0, x: -8 }} animate={{ opacity: 1, x: 0 }}
                  transition={{ delay: i * 0.04 }} className="flex items-center gap-2.5 group/todo">
                  <motion.button
                    onClick={() => { if (!t.completed) shootConfetti(); toggleDailyTodo(t.id); }}
                    whileTap={{ scale: 0.85 }}
                    className={`w-4 h-4 rounded border-2 flex items-center justify-center shrink-0 transition-all ${
                      t.completed ? 'bg-violet-500 border-violet-400' : 'border-zinc-700 hover:border-zinc-500'}`}
                  >
                    {t.completed && <motion.span initial={{ scale: 0 }} animate={{ scale: 1 }} className="text-white text-[9px] font-bold">✓</motion.span>}
                  </motion.button>
                  <span className={`text-sm flex-1 truncate ${t.completed ? 'line-through text-zinc-600' : 'text-zinc-300 group-hover/todo:text-zinc-100'}`}>
                    {t.text}
                  </span>
                </motion.li>
              ))}
          ...
          {todayDailyTodos.length === 0 && <EmptyState text="Bugün için plan yok — ekle! 🚀" />}
          ...
          {upcomingTasks.length === 0
            ...
            : <ul className="space-y-2">
                {upcomingTasks.map((task, i) => (
                  <motion.li key={task.id} initial={{ opacity: 0, x: -10 }} animate={{ opacity: 1, x: 0 }}
                    transition={{ delay: i * 0.05 }} className="flex items-center gap-3 py-1.5 group/item">
                    <div className={`w-2 h-2 rounded-full shrink-0 shadow-sm ${
                      task.priority === 'acil' ? 'bg-red-400 shadow-red-400/50' :
                      task.priority === 'yüksek' ? 'bg-orange-400 shadow-orange-400/50' : 'bg-zinc-600'}`} />
...
/* ── MAIN EXPORT — workspace-aware ── */
export default function Dashboard() {
  const { state } = useApp();
  const workspace = state.activeWorkspace || 'gunluk';
  return workspace === 'akademi' ? <AkademiDashboard /> : <GunlukDashboard />;
}
