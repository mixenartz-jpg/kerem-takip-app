...
function makeComponents(navigate) {
  const isInternalPath = (href) => href && href.startsWith('/');
      ...
      if (isInternalPath(href)) {
        return (
          <button
            ...
            className="inline-flex items-center gap-1 text-violet-400 hover:text-violet-200 bg-violet-500/10 hover:bg-violet-500/20 border border-violet-500/25 rounded-md px-2 py-0.5 text-xs font-medium transition-all cursor-pointer underline-offset-2"
          >
            {children} →
          </button>
        );
  ...
  return (
    <div className="text-zinc-300 text-sm leading-relaxed">
      <ReactMarkdown
        ...
        components={makeComponents(navigate)}
      >
        {content}
      </ReactMarkdown>
    </div>
